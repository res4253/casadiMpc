classdef casadiQpCL < casadiMpcCL
    properties
        nlp, solver,
    end
    methods
        function self = casadiQpCL(mpcModel)
           addpath('../casadi');
           import casadi.*
           ...
           max_iter = readTable('mpcConfig','max_iter');
           mu_min = readTable('mpcConfig','mu_min');
           ...
           self.solver = nlpsol('solver', 'ipopt', self.nlp, ...
               ...
               'ipopt', struct('max_iter', max_iter, 'mu_min', mu_min, 'warm_start_init_point', 'yes', 'print_level', 0, 'print_timing_statistics', 'no')));    
        end
        ...
        function  self = solveQP(self, t, x0, dist, y_ref, u_ref, ubx, lbx, ubu, lbu, Ad, Bd) 
